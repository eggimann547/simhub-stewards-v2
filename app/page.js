  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/analyze-intranet', {
        ...
        body: JSON.stringify({ url, incidentType, description }),
      });
        ...
        <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl mx-auto">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Incident Type <span className="text-red-500">(required — pick after watching)</span>
            </label>
            <select
              value={incidentType}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Short description <span className="text-gray-500">(optional — helps a lot)</span>
            </label>
            <textarea
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="e.g. White Porsche dives inside with no overlap at T3 and hits apex car"
              rows={3}
          ...
          <button
            ...
            className="w-full p-5 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 text-white font-bold text-lg rounded-xl shadow-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
          >
                ...
                <div className="space-y-6 text-gray-800 dark:text-gray-200">
                  ...
                  <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-inner space-y-6">
                    ...
                    <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-lg border-l-4 border-red-500">
                      ...
                      <p className="text-base leading-relaxed">{result.overtake_tip}</p>
                    </div>
                    <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg border-l-4 border-green-500">
                      <h4 className="font-bold text-green-700 dark:text-green-300 mb-2">Defender (Car B) – Better Defense</h4>
                      <p className="text-base leading-relaxed">{result.defend_tip}</p>
                    </div>
                  </div>
                  {/* COPY BUTTONS */}
